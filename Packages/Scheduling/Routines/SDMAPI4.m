...
CHECKO(RETURN,DFN,SD,SC,CDT) ; Check out
 ...
 ;Output:
 ; 1=Success,0=Failure 
 N CAPT,OE,APT0,CD,%,STATUS,APT0,SDCOHDL,SDATA,SDOE,SDCL,SDT
 K RETURN S RETURN=0
 ...
 Q:'RETURN 0
 S SDATA=CAPT("IFN")_U_DFN_U_SD_U_SC,SDCOHDL=$$HANDLE^SDAMEVT(1)
 D BEFORE^SDAMEVT(.SDATA,+DFN,+SD,+SC,CAPT("IFN"),SDCOHDL)
 ...
 S OE(.07)=CDT,SDOE=RETURN("SDOE"),SDCL=+SC,SDT=+SD
 D UPDOE^SDMDAL4(,.OE,RETURN("SDOE"))
 S %=$$GETCO(.RETURN,+DFN,+SD,+SC)
 D AFTER^SDAMEVT(.SDATA,+DFN,+SD,+SC,CAPT("IFN"),SDCOHDL)
 D EVT^SDAMEVT(.SDATA,5,0,SDCOHDL)
