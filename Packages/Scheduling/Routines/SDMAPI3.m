SDMAPI3 ;RGI/VSL - APPOINTMENT API; 3/19/13
...
CHKDISCH(RETURN,ENS,DFN,OENS) ; Check discharge
 ...
 S RETURN=0,SC=$O(ENS("")) Q:$G(SC)="" 0
 S %=$$GETPENRL(.OENS,+DFN,+SC)
 S ERR=0
 S TXT(1)=ENS(SC,"NAME")
 I $G(OENS(SC,"STATUS"))]"" D ERRX^SDAPIE(.RETURN,"PATDARD",.TXT) Q RETURN
 ...
 I ERR D ERRX^SDAPIE(.RETURN,"PATDHFA",.TXT) Q RETURN
 S RETURN=1
 Q RETURN
 ;
