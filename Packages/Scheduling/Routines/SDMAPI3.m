...
CHKSPCI(RETURN,DFN,SD,STATUS) ; Check if status permit check in
 ...
 Q RETURN
 ;
CHKNS(RETURN,DFN,SC,SD,LVL,STATUS,APTSTAT) ; Check no-show
 N STAT,%,APT0
 K RETURN S RETURN=1
 S:'$D(LVL) LVL=7
 S %=$$CHKPAT^SDMAPI3(.RETURN,.DFN) Q:'% 0
...
CHKOVB(RETURN,CLN,SC,SD,LEN,LVL) ; Check overbook
 N TXT,ACC,SM,MAXOB,OBNO,PP,KEYS,OUT
 S RETURN=1 S:'$G(LVL) LVL=7
 S SM=$$DECAVA(.CLN,+SC,+SD,+LEN,.PP,.OUT)
 Q:'SM 0
 S KEYS("SDOB")="",KEYS("SDMOB")=""
 D GETXUS^SDMDAL3(.ACC,.KEYS,DUZ)
 I '$D(ACC("SDOB")) S RETURN=0 D ERRX^SDAPIE(.RETURN,"APTSDOB") S RETURN(1)=OUT Q 0
 S MAXOB=CLN(1918)
 ...
 I SM=6,+LVL>1 S RETURN=0 D ERRX^SDAPIE(.RETURN,"APTOVBK",,2) S RETURN(1)=OUT Q 0
...
GETOBNO(SC,SD) ;
 ...
 Q CNT
 ;
DECAVA(CLN,SC,SD,LEN,PATT,OUT) ; Decrease availability
...
DELCOL(RETURN,DFN,SD,SC,SDDA,SDOE,OE) ; Delete check out
 ...
 D:$$HASCLS^SDMDAL4(+SDOE) DELCLS^SDMDAL4(+SDOE)
 D DELOE(+SDOE,.OE)
