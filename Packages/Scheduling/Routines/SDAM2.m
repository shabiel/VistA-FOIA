SDAM2 ;ALB/MJK - Appt Mgt (cont); 5/31/13
...
ONE(DFN,SDCL,SDT,SDDA,SDASK,SDAMCIDT) ; -- check in one appt
 ...
 I $D(XRTL) D T0^%ZOSV
 S %=$$GETAPT^SDMAPI1(.OAPT,DFN,SDT)
 S %=$$GETCLN^SDMAPI1(.CLN,SDCL)
 S ASK=(+CLN("ASK FOR CHECK IN/OUT TIME"))!SDASK
 S CIDT=$S($G(SDAMCIDT):SDAMCIDT,1:"NOW")
 I $D(OAPT) D
 . I $G(OAPT("CIDT"))]"" S ASK=1,CIDT=+OAPT("CIDT")
 . I $G(OAPT("CIDT"))="",$G(OAPT("CODT"))]"" S CIDT=+OAPT("CODT")
 S CIDT=$$TRIMSEC(CIDT)
 I $P(SDT,".")>DT S APT=0 D ERRX^SDAPIE(.APT,"APTCITS") W !!,*7,$P(APT(0),U,2) D PAUSE^VALM1 G ONEQ
 S:ASK CIDT=$$READ^SDMUI("DO^::EXTR^","CHECKED-IN",$$FTIME^VALM1(CIDT),"^D HELP^%DTC")
 I $G(CIDT)="^" W !?8,*7,"...appointment has not been checked in" D PAUSE^VALM1 G ONEQ
 D EN^DDIOL($$EZBLD^DIALOG(480000.108,$$FTIME^VALM1(CIDT)))
