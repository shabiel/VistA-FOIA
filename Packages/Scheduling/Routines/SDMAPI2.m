SDMAPI2 ;RGI/VSL - APPOINTMENT API; 3/7/13
...
MAKE(RETURN,DFN,SC,SD,TYPE,STYP,LEN,SRT,OTHR,CIO,LAB,XRAY,EKG,RQXRAY,CONS,LVL) ; Make appointment
 ...
 S:'$G(LVL) LVL=7
 K RETURN S RETURN=1
 F I="SD","LEN" I '$G(@I) S RETURN=0,TXT(1)=I D ERRX^SDAPIE(.RETURN,"INVPARAM",.TXT)
 I RETURN=0 Q 0
 S %=$$CHKPAT^SDMAPI3(.RETURN,.DFN) Q:'% 0
 S %=$$CHKCLN^SDMAPI3(.RETURN,.SC) Q:'% 0
 S %=$$CHKTYPE^SDMAPI5(.RETURN,+DFN,.TYPE) Q:'% 0
 S %=$$CHKSTYP^SDMAPI5(.RETURN,$G(TYPE),.STYP) Q:'% 0
 D GETCLN^SDMDAL1(.CLN,+SC,1)
