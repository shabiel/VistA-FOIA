SDCAPI1 ;RGI/CBR - SCHEDULING CONSULT API; 5/13/13
 ;;5.3;scheduling;**260003**;08/13/93
GETAPCNS(RETURN,DFN,STPCOD) ; Get active/pending consult requests
 N LST,FLDS,SVS,LST,TT,X1,X2,DFN1,%,COMMENT
 ...
 N A,CPRSTAT,DTENTR,DTLMT,NOS,NOSHOW,X,PTIEN,SHOW,STOP,TMP
 S NOSHOW="no-show",X1=DT,X2=-365 D C^%DTC S DTLMT=X
 S A=":"
...
SCHED(PTIEN,STPCOD,SHOW) ;===CONSULT IS SCHEDULE NOW CHECK IF IT HAS APPOINTMENT BY STOP CODE.
 N APT,APTS,SD,STOP,CLNC,CLN,STOPCOD,X,%,%DT,Y,CAPT,RETURN
 S %=$$CHKPAT^SDMAPI3(.RETURN,+PTIEN) Q:'%
 S %DT="ST",X="T-1" D ^%DT
...
CANCEL(RETURN,CONS,SC,SD,IFN,RMK,WHO,ADM,AUTO,CNDIE,CNDA) ; appt was cancelled then mark consult as edit/resubmit, add comment.
 N CAPT,CNS,CLN,BY,CONSULT,SDPATNT,%,Y
 ...
 N CPRSSTAT,SNDPRV,USER
 S CPRSSTAT=$G(CNS(8,"E")) I CPRSSTAT'="" Q:CPRSSTAT'="SCHEDULED" 0
 S SNDPRV=$G(CNS(10,"I"))
...
NOSHOW(RETURN,SC,SD,DFN,CONS,CN,AUTO,NSDIE,NSDA) ;
 ...
 N CNS,CLN,SRV,CPRSSTAT,SNDPRV,NOSHOW,CSPRT,USER,APPT,COMMENT,%,Y
 K RETURN S RETURN=0
 S %=$$CHKPAT^SDMAPI3(.RETURN,+DFN) Q:'% 0
 S %=$$CHKCLN^SDMAPI3(.RETURN,+SC) Q:'% 0
...
EDITCS(RETURN,CONS,SD,RMK,CLNC) ;Mark consult as scheduled
 N CSCHDT,SNDPRV,TME,X,Y,COMMENT,ER,CNS,%DT
 S RETURN=0
 S %DT="ST",X="NOW" D ^%DT S CSCHDT=Y
 D GETCONS^SDCDAL1(.CNS,CONS,"IE")
