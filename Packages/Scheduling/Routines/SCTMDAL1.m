SCTMDAL1 ;RGI/CBR - TEAM API; 3/21/13
...
LSTTMS(RETURN,SEARCH,START,NUMBER) ; Return teams.
 ...
 S:$D(SEARCH(0)) SCR="I $S($P(^(0),U,5)="_+SEARCH(0)_":1,1:0)"
...
GETEAM(RETURN,SCTM,INT,EXT,REZ) ; Get team
 ...
 Q
 ;
TEAMEXST(SCTM) ; Team exists?
 Q $D(^SCTM(404.51,SCTM))
 ;
GETEAMPO(RETURN,SCTM,INT,EXT,REZ) ; Get team position
 ...
 Q
 ;
TMPOEXST(SCTM) ; Team position exists?
 Q $D(^SCTM(404.57,SCTM))
 ;
...
LSTPRPO(RETURN,SEARCH,START,NUMBER) ; Return practitioner's positions.
 ...
 Q
 ;
LSTPOS(RETURN,SEARCH,START,NUMBER,FIELDS) ; Return positions.
 N FILE,RET,SCR
 S FILE="404.57" S:'$D(FIELDS) FIELDS="@;.01;.02;.09"
 S:$D(START)=0 START="" S:$D(SEARCH)=0 SEARCH=""
 S:+$G(SEARCH(0)) SCR="I $S($P(^(0),U,2)="_SEARCH(0)_":1,1:0)"
 D LIST^DIC(FILE,"",FIELDS,"",$G(NUMBER),.START,SEARCH,"B",.SCR,"","RETURN")
 Q
 ;
