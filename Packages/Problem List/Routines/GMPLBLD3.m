...
MENU ; -- init variables and list array for GMPL LIST MENU list template
 ...
 S RETURN="^TMP(""GMPLMENU1"",$J)",IND=0,IND1=0
 D GETLIST^GMPLAPI1(.RETURN,GMPLSLST,"")
 F  S IND=$O(@RETURN@("GRP",IND)) Q:'IND  D
 . S GRP=@RETURN@("GRP",IND)
 . S ^TMP("GMPLMENU",$J,IND,0)=$P(@RETURN@(GRP),U,3,4)
 . F  S IND1=$O(@RETURN@("GRP",IND,IND1)) Q:'IND1  D
 . . S PRB=@RETURN@("GRP",IND,IND1)
