GMPLDAL ; RGI/ DATA ACCESS LAYER - DIRECT GLOBALS ACCESS ; 04/09/12
...
CHGCOND(GMPIFN,OLD,NEW,GMPROV) ; Change condition flag
 ...
 S AUDMSG="^"_OLD_"^"_NEW_"^"
 S:(NEW="H") AUDMSG=AUDMSG_"Deleted^"
 S:(OLD="T")&(NEW="P") AUDMSG=AUDMSG_"Verified^"
 S:(OLD="H")&(NEW="P") AUDMSG=AUDMSG_"Replaced^"
...
LOCK(GMPIFN,SUB,ERT) ;
 ...
 I 'LOCKED D ERR^GMPLAPIE(ERT,"FILELOCK")
 Q LOCKED
 ;
UNLOCK(GMPIFN,SUB) ;
...
GETPLIST(PLIST,TOTAL,GMPDFN,GMPSTAT,GMPREV,GMPROV,GMPVIEW,GMPIDX) ; Build PLIST(#)=IFN for view
 N SHOWDEL,ST,STEND,IFN,RECORD,LIST,JUSTDEL
 K PLIST
