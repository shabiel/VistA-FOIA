...
NEW(RETURN,GMPDFN,GMPROV,GMPFLD) ; Save Collected Values in new Problem Entry
 ...
 . D NEWNOTE^GMPLAPI3(.RETURN,GMPIFN,GMPROV,.NOTES)
 S RETURN=GMPIFN
 Q 1
 ;
...
DELETE(RETURN,GMPIFN,GMPROV,REASON) ; DELETE A PROBLEM
 ...
 I $G(REASON)'="" D NEWNOTE^GMPLAPI3(.RETURN,GMPIFN,GMPROV,REASON)
...
DETAIL(RETURN,GMPIFN,GMPLMGR,GMPROV) ; 
 ...
 Q 1
 ;
DETAILX(RETURN,GMPIFN,GMPLMGR,GMPROV) ; Returns Formatted Detailed Data for Problem
 ;                
 ...
 S GMPLMGR=$G(GMPLMGR)
 S GMPROV=$G(GMPROV)
 Q:'$$DETAIL(.GMPFLD,GMPIFN,GMPLMGR,GMPROV)
...
INACTV(RETURN,GMPIFN,GMPROV,NOTE,RESOLVED) ; Inactivate problem
 ...
 D:$G(NOTE)'="" NEWNOTE^GMPLAPI3(.RETURN,GMPIFN,GMPROV,NOTE)
 S DIE="^AUPNPROB(",DR=".12///I;1.07////"_$P($G(RESOLVED),U)
 S DA=GMPIFN D ^DIE
 S CHNGE=GMPIFN_"^.12^"_$$HTFM^XLFDT($H)_U_DUZ_"^A^I^Inactivated^"_+$G(GMPROV)
 D AUDIT^GMPLX(CHNGE,"")
