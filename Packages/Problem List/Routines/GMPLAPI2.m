...
NEW(RETURN,GMPDFN,GMPROV,GMPVAMC,GMPFLD) ; Save Collected Values in new Problem Entry
 ; Input
 ...
 . D NEWNOTE^GMPLAPI3(GMPIFN,GMPROV,GMPVAMC,.NOTES,"RETURN")
 S RETURN=GMPIFN
 Q 1
 ;
UPDATE(RETURN,GMPIFN,GMPORIG,GMPFLD,GMPLUSER,GMPVAMC,GMPROV) ; Save Changes made to Existing Problem
 ...
 F I=0:0 S I=$O(GMPORIG(10,I)) Q:I'>0  D UPDNOTE^GMPLAPI3(GMPIFN,GMPORIG(10,I),GMPFLD(10,I),GMPROV,"RETURN")
 I $D(GMPFLD(10,"NEW"))>9 D 
 . M NOTES=GMPFLD(10,"NEW")
 . D NEWNOTE^GMPLAPI3(GMPIFN,GMPROV,GMPVAMC,.NOTES,"RETURN")
 ; Quit Saving Changes
...
DELETE(RETURN,GMPIFN,GMPROV,GMPVAMC,REASON) ; DELETE A PROBLEM
 ...
 I $G(REASON)'="" D NEWNOTE^GMPLAPI3(GMPIFN,GMPROV,GMPVAMC,REASON,"RETURN")
 S RETURN=$$DELETE^GMPLDAL(GMPIFN,GMPROV,"RETURN")
 Q RETURN
 ;
DETAIL(RETURN,GMPIFN,GMPLMGR,GMPVAMC,GMPROV) ; 
...
VERIFY(RETURN,GMPIFN) ; Verify a transcribed problem
 ...
 Q 1
 ;
DUPL(RETURN,DFN,TERM,TEXT) ; Check's for Duplicate Entries
 N IFN,NODE0,NODE1
 S RETURN=0,TEXT=$$UP^XLFSTR(TEXT)
 I '$D(^AUPNPROB("C",TERM))!('$D(^AUPNPROB("AC",DFN))) Q 1
 F IFN=0:0 S IFN=$O(^AUPNPROB("AC",DFN,IFN)) Q:IFN'>0  D  Q:RETURN>0
 ...
 Q 1
 ;
CODESTS(RETURN,PROB,ADATE) ;check status of code associated with a problem
 ; Input:
 ...
 S RETURN=+($$STATCHK^ICDAPIU($$CODEC^ICDCODE(+(PROB)),ADATE))
 Q RETURN
 ;
...
INACTV(RETURN,GMPIFN,GMPROV,GMPVAMC,NOTE,RESOLVED) ; Inactivate problem
 ...
 D:$G(NOTE)'="" NEWNOTE^GMPLAPI3(GMPIFN,GMPROV,GMPVAMC,NOTE,"RETURN")
