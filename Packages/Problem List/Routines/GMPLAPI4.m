GMPLAPI4 ; RGI/CBR -- Problem List API - LIST ;3/21/13
...
LIST(RETURN,GMPDFN,GMPSTAT,GMPROV,GMPVIEW,GMPREV,GMPIDX,ORICD186) ;
 ...
 N PLIST
 K RETURN
 I '$$GETPLIST(.PLIST,.GMPDFN,.GMPSTAT,.GMPREV,.GMPROV,.GMPVIEW,.GMPIDX) M RETURN=PLIST Q 0
...
BUILDLST(RETURN,GMPLIST,ORICD186) ; Same as LIST but returns only problems passed in GMPLIST
 ...
 S RETURN=0
 I '$D(GMPLIST) D ERRX^GMPLAPIE(.RETURN,"INVPARAM","GMPLIST")
 I '$$NUM^GMPLCHK(.RETURN,$G(GMPLIST(0)),"GMPLIST",0,0,1)
 F I=1:1:GMPLIST(0) D
 . Q:'$D(GMPLIST(I))
 . S INACT=""
 . S IFN=+GMPLIST(I)
...
REPLACE(RETURN,GMPIFN,NEWDIAG) ; Replace ICD diagnosis code
 ...
 S RETURN=0
 I '$$PRBIEN^GMPLCHK(.RETURN,.GMPIFN) Q 0
 I '$$ICDCODE^GMPLCHK(.RETURN,.NEWDIAG) Q 0
