...
LIST(GMPL,GMPDFN,GMPSTAT,GMPCOMM) ; Returns list of Prob for Pt.           
 ...
 D LIST^GMPLAPI4(.GMPL,GMPDFN,GMPSTAT,0,"",GMPARAM("REV"))
 F CNT=1:1:GMPL(0) D
 . S IFN=$P(GMPL(CNT),U)
...
DETAIL(IFN,GMPL) ; Returns Detailed Data for Problem
 ...
 ;              
 N VALID
 D VALID^GMPLAPI4(.VALID,IFN)
 Q:'VALID
 D DETAILX^GMPLAPI2(.GMPL,IFN)
 K GMPL("HISTORY")
 D AUDIT
 Q
 ;
...
ADD(DFN,LOC,GMPROV) ; -- Interactive LMgr action to add new problem
 ...
 D GETULST^GMPLAPI6(.RETURN,DUZ) S GMPLSLST=RETURN
 I 'GMPLSLST,GMPCLIN D GETCLST^GMPLAPI6(.RETURN,+GMPCLIN) S GMPLSLST=RETURN
 I GMPLSLST D  Q
...
REMOVE(GMPIFN,GMPROV,TEXT,PLY) ; -- Remove problem GMPIFN
 ...
 D DELETE^GMPLAPI2(.PLY,GMPIFN,GMPROV,$G(TEXT))
 S:PLY PLY=GMPIFN
 Q
 ;
