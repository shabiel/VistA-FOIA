...
ADD1 ; set up default values
 ...
 D ELIG(.GMPELIG)
 I '$$DEFAULT^GMPLAPI2(.GMPFLD,GMPROB,GMPICD,GMPTERM,$G(GMPROV),$G(GMPCLIN),.GMPELIG) D  H 3 Q
...
STATUS(GMPIFN,GMPROV,GMPVAMC,GMPSAVED,ERT) ; -- inactivate problem
 ...
 W !
 D INACTV^GMPLAPI2(.RETURN,GMPIFN,$G(GMPROV),GMPVAMC,Y,$G(GMPFLD(1.07)))
 W "... inactivated!",!
 H 1
 S GMPSAVED=1
 Q 1
 ;
NEWNOTE(RETURN,GMPIFN,GMPROV,GMPVAMC) ; -- add a new comment
 ...
 Q $$NEWNOTE^GMPLAPI3(.RETURN,GMPIFN,GMPROV,GMPVAMC,.NOTES)
 ;
DELETE(RETURN,GMPIFN,GMPROV,GMPVAMC,GMPSAVED) ; -- delete a problem
 ...
 S SAVED=$$DELETE^GMPLAPI2(.RETURN,GMPIFN,GMPROV,GMPVAMC,Y)
 W:SAVED "...... removed!"
 W ! H 1
 S:SAVED GMPSAVED=1
 Q SAVED
...
VERIFY ; -- verify a transcribed problem, if parameter on
 N GMPERR,VERIFIED,ICDACTV
 W !!,$$PROBTEXT^GMPLX(GMPIFN),!
 D CODESTS^GMPLAPI2(.ICDACTV,GMPIFN,DT)
 I 'ICDACTV W "has an inactive ICD9 code. Edit the problem before verification.",! H 2 Q
 D VERIFIED^GMPLAPI2(.VERIFIED,GMPIFN)
 I VERIFIED W "does not require verification.",! H 2 Q
