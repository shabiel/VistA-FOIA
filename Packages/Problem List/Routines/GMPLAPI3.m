GMPLAPI3 ; / Problem List API - NEW,UPDATE NOTES ; 03/29/12
...
UPDNOTE(RETURN,GMPIFN,OLDNOTE,NEWNOTE,GMPROV) ;
 ...
 Q RETURN
 ;
NOTES(RETURN,GMPIFN,GMPACT)       ; Return comments for a problem - MULTI-DIVISIONAL
 ; GMPACT - Active only
 ...
 . . I GMPACT,$P($G(^AUPNPROB(GMPIFN,11,FAC,11,NIFN,0)),U,4)'="A" Q
 . . S NOTE=$P($G(^AUPNPROB(GMPIFN,11,FAC,11,NIFN,0)),U,3)
 . . S NOTECNT=NOTECNT+1,RETURN(NOTECNT)=NOTE
 S RETURN=NOTECNT
 Q 1
 ;
FULLNTE(RETURN,GMPIFN) ;Get active comments for a note
 ...
 K RETURN
 S RETURN=0
 Q:$D(^AUPNPROB(GMPIFN,11))'>0 1
 S FAC=0
 F  S FAC=$O(^AUPNPROB(GMPIFN,11,FAC)) Q:FAC'>0  D
 . Q:$D(^AUPNPROB(GMPIFN,11,FAC,11))'>0
 . S LOC=+$G(^AUPNPROB(GMPIFN,11,FAC,0))
 . S NIFN=0
 . F  S NIFN=$O(^AUPNPROB(GMPIFN,11,FAC,11,NIFN)) Q:NIFN'>0  D
 . . S NODE=$G(^AUPNPROB(GMPIFN,11,FAC,11,NIFN,0))
 . . Q:$P(NODE,U,4)']""  ; Only active comments
 . . S RETURN(LOC,$P(NODE,U))=NODE
 . . S RETURN=RETURN+1
 Q 1
