DGPMDAL2 ;RGI/VSL - PATIENT MOVEMENT DAL; 7/11/13
...
LSTWARD(RETURN,SEARCH,START,NUMBER,FLDS) ; Return wards.
 ...
 Q
 ;
LSTWBED(RETURN,SEARCH,START,NUMBER,FLDS,WARD,DFN) ; Return ward beds.
 N SCR,TMP,E
 S:'$D(FLDS) FLDS=".01;.02;.2;"
 S FLDS="@;"_FLDS
 S SCR="I $D(^DG(405.4,""W"",+WARD,+Y)),'$$ISBEDOCC^DGPMAPI9(+Y,+$G(DFN))"
...
LSTADSRC(RETURN,SEARCH,START,NUMBER,FLDS,DGDT) ; Return facility treating specialties
 ...
 S FLDS="@;"_FLDS S S=$S('$D(SEARCH):"",1:SEARCH)
 I +$E(S,1)>0 S IND="B"
 E  S IND="C"
 D LIST^DIC(45.1,"",FLDS,"",.NUMBER,.START,.SEARCH,IND,,"","RETURN","E")
 I $D(E) M RETURN=E
 Q
 ;
...
LSTMVTT(RETURN,SEARCH,START,NUMBER,FLDS,TYPE,DFN,DGDT) ; Return movement types.
 ...
 N %,LMVT,MAS,NMVT,PAR,PMVT,DGAPIDT
 K ^UTILITY($J,"DGAPIDT")
 S:'$D(FLDS) FLDS=".01;.02IE;.04IE"
 S FLDS="@;"_FLDS
 S (DGPM0,DGPM2,DGPMP,DGPMAN)="",DGPMDA=0,SEARCH=$E($G(SEARCH),1,29)
 D GETLASTM^DGPMDAL1(.LMVT,+DFN,+DGDT)
 S:+DGDT=+LMVT(3) MFN=LMVT(1),DGPMDA=MFN,DGPMP=$G(^DGPM(MFN,0))
 S PAR("DATE")=+DGDT,^UTILITY($J,"DGAPIDT")=+DGDT S:LMVT(13) DGPMAN=$G(^DGPM(LMVT(13),0))
 S DGPMCA=LMVT(13)
 S %=$$GETMVT^DGPMAPI8(.ADM,LMVT(13)) ;,DGPMP=$G(^DGPM(LMVT(13),0))
 S %=$$GETPMVT^DGPMAPI9(.PMVT,LMVT(13),DGDT,.MFN)
 S:$G(PMVT("ID")) DGPM0=$G(^DGPM(PMVT("ID"),0))
 S %=$$GETNMVT^DGPMAPI9(.NMVT,LMVT(13),DGDT,.MFN)
 S:$G(NMVT("ID")) DGPM2=$G(^DGPM(NMVT("ID"),0))
