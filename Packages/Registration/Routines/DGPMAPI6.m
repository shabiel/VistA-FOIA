DGPMAPI6 ;RGI/VSL - FACILITY TRATING SPECIALTY API; 4/18/13
...
UPDFTS1(RETURN,PARAM,MFN,PC) ; Update specialty transfer
 ...
 I OLD=0 S TXT(1)=$$EZBLD^DIALOG(4070000.01_(DGPMT+$G(PC))) D ERRX^DGPMAPIE(.RETURN,"MVTNFND",.TXT) Q 0
...
CHKDT(RETURN,PARAM)
 ...
 Q 1
PROVCH(RETURN,PARAM) ; Add provider change
 Q $$FTS1(.RETURN,.PARAM,1)
 ;
FTS(RETURN,PARAM) ; Add facility treating movement
 Q $$FTS1(.RETURN,.PARAM)
 ;
...
EVT(DFN,IFN,MODE) ; Event
 ...
 Q
 ;
ADD(RETURN,PARAM,PC) ; Add ralated physical movement
 N %,PM6,MFN,DIAG,PTS
 I $G(PC) D
