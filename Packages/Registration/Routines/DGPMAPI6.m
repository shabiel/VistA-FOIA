DGPMAPI6 ;RGI/VSL - FACILITY TRATING SPECIALTY API; 4/8/13
 ;;5.3;Registration;**260005**;
CHKADD(RETURN,PARAM) ; 
 N % K RETURN S RETURN=1
 ; facility treating specialty
 I $G(PARAM("DGPMPC"))'="",$G(PARAM("DGPMPC"))'=1 D  Q 0
 . S RETURN=0,TXT(1)="PARAM('DGPMPC')" D ERRX^DGPMAPIE(.RETURN,"INVPARM",.TXT)
 ...
 Q 1
 ;
CHKUPD(RETURN,PARAM,MFN,OLD,NEW) ; 
 N %,I,OLD
 K RETURN S RETURN=1
 D GETMVT^DGPMAPI8(.OLD,MFN)
 S PARAM("PATIENT")=OLD("PATIENT")
 ; facility treating specialty
 I '$G(PARAM("DGPMPC")),$G(PARAM("FTSPEC"))'="",+OLD("FTSPEC")'=+PARAM("FTSPEC") D  Q:'RETURN 0
...
UPDFTSE(RETURN,PARAM,MFN) ; Update facility treating specialty
 N %,OLD,NEW,DFN
 K RETURN S RETURN=0,DGPMT=6
 S %=$$GETMVT^DGPMAPI8(.OLD,+$G(MFN))
 I OLD=0 S TXT(1)=$$EZBLD^DIALOG(4070000.01_(DGPMT+$G(PARAM("DGPMPC")))) D ERRX^DGPMAPIE(.RETURN,"MVTNFND",.TXT) Q 0
 S %=$$CHKUDT^DGPMAPI2(.RETURN,.PARAM,MFN,.OLD,.NEW)
 S:RETURN %=$$CHKUPD(.RETURN,.PARAM,.MFN,.OLD,.NEW)
 I RETURN=0 S:'$D(RETURN(0)) RETURN=1 Q $S('$D(RETURN(0)):1,1:0)
 S DFN=$P(OLD("PATIENT"),U)
 S %=$$LOCKMVT^DGPMAPI9(.RETURN,DFN) Q:'RETURN 0
 S %=$$UPD(.RETURN,.PARAM,MFN,.OLD,.NEW)
...
DELFTS(RETURN,MFN) ; Delete facility treating movement
 N %,MVT,DFN
 K RETURN S RETURN=0,DGPMT=6,DA=+MFN
