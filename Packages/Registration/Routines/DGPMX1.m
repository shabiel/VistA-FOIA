DGPMX1 ; GENERATED FROM 'DGPM ADMIT' INPUT TEMPLATE(#446), FILE 405;3/4/2013
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:'$D(OLD)
 S %=$P($G(OLD("DATE")),U) S:%]"" DE(2)=%
 S %=$P($G(OLD("TYPE")),U) S:%]"" DE(13)=%
 S %=$P($G(OLD("ADMSCC")),U) S:%]"" DE(10)=%
 S %=$P($G(OLD("ADMREG")),U) S:%]"" DE(5)=%
 S %=$P($G(OLD("FDEXC")),U) S:%]"" DE(4)=%
 S %=$P($G(OLD("ADMCAT")),U) S:%]"" DE(7)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="DGPMX1",DQ=1
 ...
 S:$D(DTIME)[0 DTIME=300 S DIEZ=446,U="^"
1 S Y=U,DQ=1 D X1 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X1 S:$S(DGPMN:1,DGPMY=+OLD("DATE"):1,1:0) Y=41
 Q
...
X2 S %DT="ST" D ^%DT K %DT S X=Y K:Y<1 X I $D(X),'$D(DGPMT) W !?3,*7,"USE BED CONTROL MOVEMENT OPTIONS!" K X
 ...
 ;
3 S Y=U,DQ=3 G A
4 S DQ=4,DW="DIR;1",DV="SXR",DU="",DLB="DOES THE PATIENT WISH TO BE EXCLUDED FROM THE FACILITY DIRECTORY?",DIFLD=41
 S DE(DW)="C4^DGPMX1"
 S DU="0:NO;1:YES;"
 G RE
...
X4 I $D(X),'$D(DGPMT) D EN^DDIOL("USE BED CONTROL MOVEMENT OPTIONS!",,"!") K X
 Q
 ;
5 S PAR("FDEXC")=$S(Y="YES":1,1:0),DQ=5,DW="0;12",DV="R*P43.4'",DU="",DLB="ADMITTING REGULATION",DIFLD=.12
 S DU="DIC(43.4,"
 G RE
X5 S DIC("S")="I '$P(^(0),""^"",4)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
6 S PAR("ADMREG")=$S(+Y>0:"",1:X_U)_Y,Y=U,DQ=6 G A
7 S DW="PTF;4",DV="P35.2'",DU="",DLB="ADMITTING CATEGORY",DIFLD=54
 S DU="DG(35.2,"
 S X=$$ADCAT^DGSAUTL(+PAR("ADMREG"))
 S Y=X
 S X=Y,PAR("ADMCAT")=Y G:X="" A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
X7 Q
8 S Y=U,DQ=8 D X8 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
...
X10 Q
11 S DQ=12 ;@11
12 S PAR("ADMSCC")=$S(Y="^":"",Y="YES":1,1:0),Y=U,DQ=12 D X12 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
...
X13 S DIC("S")="I $D(DGPMT),($P(^(0),""^"",2)=DGPMT),$P(^(0),""^"",4) S DGER=0,DGPMTYP=$P(^(0),""^"",3) D:DGPMT<4!(DGPMT=6)!(DGPMT=5) @(""DICS^DGPMV3""_DGPMT) I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
14 S PAR("TYPE")=$S(+Y>0:"",1:X_U)_Y,Y=U,DQ=14 D X14 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X14 S ^DISV(DUZ,"DGPM1")=$S($D(^DISV(DUZ,"^DG(405.1,")):^("^DG(405.1,"),1:"")
 Q
15 S Y=U,DQ=15 D X15 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
X15 I $P(^DG(405.1,+PAR("TYPE"),0),"^",3)'=9 S Y=.1
 Q
16 G ^DGPMX12
