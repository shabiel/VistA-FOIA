DGSAUTL ;ALB/MTC - SHARING AGREEMENTS UTILITY FUNCTIONS ;8/26/13
...
GOGO(ORG,ATR,CAT) ;-- This function does something
 ...
 I ORG="SD" D
 . S %=$$LSTASTYP^SDMAPI5(.STYP,ATR)
 I ORG="DG" D
 . S %=$$LSTACAT^DGSAAPI(.STYP,ATR)
 F IN=0:0 S IN=$O(STYP(IN)) Q:'IN  S:+STYP(IN,"SUBCAT")=+CAT DA=STYP(IN,"ID")
 I DA D
...
HLP ;-- help for Sub-Category file
 ...
 ;
 N %,DGX,DGI,DGJ,TYPE,STYP
 I DGORG="SD" D
 . S %=$$GETAPPT^SDMAPI1(.TYPE,DGAPT)
 . S %=$$LSTASTYP^SDMAPI5(.STYP,DGAPT)
 I DGORG="DG" D
 . S %=$$GETAREG^DGPMAPI8(.TYPE,DGAPT)
 . S %=$$LSTACAT^DGSAAPI(.STYP,DGAPT)
 S DGJ=1
 F DGI=0:0 S DGI=$O(STYP(DGI)) Q:'DGI  D
 . W:DGJ !,$S(DGORG="SD":"APPOINTMENT TYPE :",1:"VA ADMITTING REGULATION :")
 . W:DGJ TYPE("NAME"),!,?5,"CATEGORY :" S:DGJ DGJ=0
 . W !,?10,$P(STYP(DGI,"SUBCAT"),U,2),?35,$S(STYP(DGI,"SUBCAT"):"ACTIVE",1:"INACTIVE")
HLPQ ;
 Q
 ;
