DGPMV33 ;ALB/MIR - DISCHARGE A PATIENT, CONTINUED ; 8/26/13
...
DICS ;input transform on discharge type
 ...
 S DGPMAN=$G(^DGPM(+DGPMCA,0)) I DGPMAN="" S DGER=1 Q
 ;I "^21^47^48^49^"[("^"_DGX1_"^") S DGER=1 Q
 I DGX1=42,'$O(^DGPM("ATID2",+DFN,9999999.9999999-$S($D(^UTILITY($J,"DGAPIDT")):+^UTILITY($J,"DGAPIDT"),1:+^DGPM(DA,0)))) S DGER=1 Q
 S DGX=+$P(DGPMP,"^",18) I DGX,"^41^46^"[("^"_DGX_"^"),(DGX1'=DGX) S DGER=1 Q
 I "^42^47^"[("^"_DGX1_"^"),(DGX1'=$P(DGPMP,"^",18)) S DGER=1 Q
 I "^42^47^"[("^"_DGX_"^"),(DGX1'=$P(DGPMP,"^",18)) S DGER=1 Q
 ...
 D SET^DGPMV32 S X1=+DGPMAB,X2=30,DGHX=X D C^%DTC I DGPMP>X S DGER=1,X=DGHX K DGHX Q
