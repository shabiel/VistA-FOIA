DGPMAPI7 ;RGI/VSL - PATIENT MOVEMENT API; 3/4/2013
...
LSTADSRC(RETURN,SEARCH,START,NUMBER) ; Return source of admission
 ...
 Q 1
 ;
LSTCOTYP(RETURN,SEARCH,START,NUMBER,DFN,DGDT) ; Return check-out types
 S %=$$BLDTYP(.RETURN,.SEARCH,.START,.NUMBER,.DFN,.DGDT,5) Q:'% 0
 Q 1
 ;
LSTCITYP(RETURN,SEARCH,START,NUMBER,DFN) ; Return check-in types
 S %=$$BLDTYP(.RETURN,.SEARCH,.START,.NUMBER,.DFN,$$NOW^XLFDT(),4) Q:'% 0
 Q 1
 ;
LSTADTYP(RETURN,SEARCH,START,NUMBER,DFN) ; Return admission types
 S %=$$BLDTYP(.RETURN,.SEARCH,.START,.NUMBER,.DFN,$$NOW^XLFDT(),1) Q:'% 0
 Q 1
 ;
LSTTRTYP(RETURN,SEARCH,START,NUMBER,DFN,DGDT) ; Return transfer types
 S %=$$BLDTYP(.RETURN,.SEARCH,.START,.NUMBER,.DFN,.DGDT,2) Q:'% 0
 Q 1
 ;
LSTDTYP(RETURN,SEARCH,START,NUMBER,DFN,DGDT) ; Return discharge types
 S %=$$BLDTYP(.RETURN,.SEARCH,.START,.NUMBER,.DFN,.DGDT,3) Q:'% 0
 Q 1
 ;
BLDTYP(RETURN,SEARCH,START,NUMBER,DFN,DGDT,TYP) ; Build movement types
 N TXT
 S %=$$CHKPAT^DGPMAPI8(.RETURN,$G(DFN)) Q:'RETURN 0
 I $G(DGDT)=""!(+$G(DGDT)<1800000) D  Q 0
 . S TXT(1)="DGDT",RETURN=0 D ERRX^DGPMAPIE(.RETURN,"INVPARM",.TXT)
 N FLDS,NAMES,TYPES
 K RETURN S FLDS=".01;.02;.04"
 S NAMES="NAME;TYPE;STATUS",SEARCH=$$UP^XLFSTR($G(SEARCH))
 D LSTMVTT^DGPMDAL2(.TYPES,.SEARCH,.START,.NUMBER,FLDS,TYP,+DFN,.DGDT)
...
LSTWBED(RETURN,SEARCH,START,NUMBER,WARD,DFN) ; Return beds
 ...
 I $G(DFN)>0 S %=$$CHKPAT^DGPMAPI8(.RETURN,$G(DFN)) Q:'RETURN 0
...
MVTEVT(DFN,TYPE,MFN,QUIET) ; Movement events
 ...
 S DGPMDA=MFN,DGPMP="",DGPMT=TYPE,DGQUIET=0 ;$S($G(QUIET):0,1:1)
 S:TYPE=1 DGNEW=1
 D ^DGPMEVT
 Q
 ;
UPDPAT(RETURN,PARAM,DFN) ; Update patient
 N PAT
 S:+$G(PARAM("FTSPEC")) PAT(.103)=+$G(PARAM("FTSPEC"))
 S:+$G(PARAM("PRYMPHY")) PAT(.104)=$S(+$G(PARAM("PRYMPHY"))>0:+$G(PARAM("PRYMPHY")),1:+$G(PARAM("ATNDPHY")))
 S:+$G(PARAM("ATNDPHY")) PAT(.1041)=+$G(PARAM("ATNDPHY"))
 S:$G(PARAM("SERILL"))]"" PAT(401.3)=$G(PARAM("SERILL"))
 S:$G(PARAM("SERILL"))]"" PAT(401.4)=$S($G(PARAM("SERILL"))="":"",1:$P($$NOW^XLFDT(),"."))
 D RESET^DGPMDDCN
