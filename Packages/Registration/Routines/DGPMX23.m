...
1 S DQ=1,DW="0;13",DV="DX",DU="",DLB="ABSENCE RETURN DATE",DIFLD=.13
 S Y="@"
...
X7 K:'$D(DGPMT) X I $D(X) S DIC("S")="I $D(^DG(405.4,""W"",+PAR(""WARD""),+Y)) D OCC^DGPMRB I 'DGPMOC" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D ROOM^DGPMVDD K:$D(DGOOS) X K DGOOS
 S:$D(X) PAR("ROOMBED")=$S(+Y>0!(Y="^"):"",Y<0:"",1:X_U)_Y
 Q
 ;
8 S DQ=9 ;@25
...
X9 ;I DGPMP=^DGPM(DA,0) S Y=""
 Q
10 S DQ=10,DW="USR;3",DV="RP200'",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DU="VA(200,"
 S X=DUZ
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X12 S %DT="STX" D ^%DT S X=Y K:Y<1 X
 Q
 ;
13 S Y=U,DQ=13 D X13 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
