DGPMX2 ; GENERATED FROM 'DGPM TRANSFER' INPUT TEMPLATE(#447), FILE 405;3/4/2013
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:'$D(OLD)
 S %=$P($G(OLD("DATE")),U) S:%]"" DE(2)=%
 S %=$P($G(OLD("TYPE")),U) S:%]"" DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S Y=U,DQ=1 D X1 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
...
3 S Y=U,DQ=3 G A
4 S DQ=5 ;@22
5 S Y=U,DQ=5 D X5 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
X5 S ^DISV(DUZ,"^DG(405.1,")=$S($D(^DISV(DUZ,"DGPM2")):^("DGPM2"),1:"")
 Q
6 S DQ=6,DW="0;4",DV="R*P405.1'X",DU="",DLB="TYPE OF TRANSFER",DIFLD=.04
 S DE(DW)="C6^DGPMX2"
 S DU="DG(405.1,"
 G RE
...
X6 S DIC("S")="I $D(DGPMT),($P(^(0),""^"",2)=DGPMT),$P(^(0),""^"",4) S DGER=0,DGPMTYP=$P(^(0),""^"",3) D:DGPMT<4!(DGPMT=6)!(DGPMT=5) @(""DICS^DGPMV3""_DGPMT) I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
7 S PAR("TYPE")=$S(+Y>0:"",1:X_U)_Y,Y=U,DQ=7 D X7 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
X7 S ^DISV(DUZ,"DGPM2")=$S($D(^DISV(DUZ,"^DG(405.1,")):^("^DG(405.1,"),1:"")
 Q
8 S Y=U,DQ=8 D X8 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
X8 S DGZ=$P(^DG(405.1,+PAR("TYPE"),0),"^",3) I $S(DGZ<4:0,DGZ<22:1,DGZ<27:0,DGZ=44:1,1:0) K DGZ S Y="@2"
 Q
9 S Y=U,DQ=9 D X9 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
