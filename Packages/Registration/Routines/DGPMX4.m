DGPMX4 ; GENERATED FROM 'DGPM CHECK-IN LODGER' INPUT TEMPLATE(#450), FILE 405;3/4/2013
 D DE G BEGIN
...
X10 K:'$D(DGPMT) X I $D(X) S DIC("S")="I $D(^DG(405.4,""W"",+PAR(""WARD""),+Y)) D OCC^DGPMRB I 'DGPMOC" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D ROOM^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
11 S PAR("ROOMBED")=$S(+Y>0:"",1:X_U)_Y,Y=U,DQ=11 D X11 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
