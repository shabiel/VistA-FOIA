DGPMAPI8 ;RGI/VSL - PATIENT MOVEMENT API; 3/28/2013
...
GETLASTM(RETURN,DFN,DGDT) ; Get last patient movement
 ...
 ;I LMVT(17)'=LMVT(1) D
 ;. D GETMVT^DGPMDAL1(.MVT,+LMVT(17),".01")
...
GETADM(RETURN,AFN) ; Get admission
 ...
 I MVT=0 M RETURN=MVT S RETURN=0 D ERRX^DGPMAPIE(.RETURN,"ADMNFND") Q 0
...
GETTRA(RETURN,TFN) ; Get transfer
 ...
 I MVT=0 M RETURN=MVT S RETURN=0 D ERRX^DGPMAPIE(.RETURN,"TRANFND") Q 0
 D BUILD(.RETURN,.MVT,FLDS,NAMES)
 D GETPAT^DGPMDAL2(.PAT,+RETURN("PATIENT"),"401.3")
 S RETURN("SERILL")=PAT(401.3,"I")_U_PAT(401.3,"E")
 S FLDS=".08;.09;.19"
...
GETMVT(RETURN,MFN) ; Get movement
 ...
 S NAMES="DATE;TTYPE;PATIENT;TYPE;FCTY;WARD;ROOMBED;PRYMPHY;FTSPEC;SHDIAG;ADMIFN;DISCH;MASTYPE;ATNDPHY;RPM;LDGRSN;LDGCOMM;LDGDISP;"
 D GETMVT^DGPMDAL1(.MVT,+MFN,FLDS)
 I MVT=0 M RETURN=MVT S RETURN=0 D ERRX^DGPMAPIE(.RETURN,"MVTNFND") Q 0
 D BUILD(.RETURN,.MVT,FLDS,NAMES)
...
GETMVTT(RETURN,IFN) ; Get movement type
 ...
 I MVT=0 M RETURN=MVT S RETURN=0 D ERRX^DGPMAPIE(.RETURN,"MVTTNFND") Q 0
 D BUILD(.RETURN,.MVT,FLDS,NAMES)
 S RETURN=1
 Q 1
 ;
GETPSRV(RETURN,IFN) ; Get period of service
 ...
 I MVT=0 M RETURN=MVT S RETURN=0 D ERRX^DGPMAPIE(.RETURN,"PSRVNFND") Q 0
 D BUILD(.RETURN,.MVT,FLDS,NAMES)
 S RETURN=1
 Q 1
 ;
...
CHKASH(RETURN,PARAM,DFN,MAS) ;
 N %,WRD
 S RETURN=0
 S %=$$CHKADD^DGPMAPI1(.RETURN,.PARAM,1) Q:'RETURN&($P($G(RETURN(0)),U)'="WRDCNASB") 0
 K RETURN
