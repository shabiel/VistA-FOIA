DGPMV331 ;ALB/MIR - ASIH DISCHARGE PROCESSING ; 5/16/13
 ;;5.3;Registration;;Aug 13, 1993
ASIH ;if admission type was TO ASIH...
 N TMP,MVT,ADM1
 M ADM1=ADM
 S %=$$GETMVT^DGPMAPI8(.MVT,$G(ADM("ASIHTRA")))
 S %=$$GETMVT^DGPMAPI8(.ADM,$G(MVT("ADMIFN")))
 S TRMAS=$S(+$G(MVTT("MAS"))=46:45,+$G(MVTT("MAS"))=41:14,1:0)
 S TYP=$$GETAMT^DGPMDAL3(TRMAS),%=$$GETMVTT^DGPMAPI8(.TT,MVTT("MAS"))
 M TMP=PAR S PAR("TYPE")=TYP,DGPMT=2,DIE("NO^")="",DP=405,DIEZ=447
 N ASHDCH
 I +$G(MVTT("MAS"))=46 S ASHDCH=1 D 8^DGPMX22
 D ^DGPMX23:+$G(MVTT("MAS"))'=46
 I $G(TT("ASKSPEC")) S Y="^0" D SPEC^DGPMV36
 S DGPMT=3 M PAR=TMP,ADM=ADM1
 Q
