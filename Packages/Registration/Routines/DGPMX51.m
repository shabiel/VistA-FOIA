...
1 S DQ=1,DW="0;4",DV="R*P405.1'X",DU="",DLB="CHECK-OUT TYPE",DIFLD=.04
 S DE(DW)="C1^DGPMX51"
 S DU="DG(405.1,"
 G RE
...
X1 S DIC("S")="I $D(DGPMT),($P(^(0),""^"",2)=DGPMT),$P(^(0),""^"",4) S DGER=0,DGPMTYP=$P(^(0),""^"",3) D:DGPMT<4!(DGPMT=6)!(DGPMT=5) @(""DICS^DGPMV3""_DGPMT) I 'DGER" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 S PAR("TYPE")=$S(+Y>0:"",1:X_U)_Y
 Q
 ;
2 S Y=U,DQ=2 D X2 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
X2 S ^DISV(DUZ,"DGPM5")=$S($D(^DISV(DUZ,"^DG(405.1,")):^("^DG(405.1,"),1:"")
 Q
3 S DQ=4 ;@53
4 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=4 D X4 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
X4 S:DGPMN DIE("NO^")=""
 Q
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="LD;3",DV="RS",DU="",DLB="DISPOSITION",DIFLD=30.03
 ...
 G RE
X5 Q
6 S PAR("LDGDISP")=X_U_Y,Y=U,DQ=6 D X6 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
X6 I $G(OLD("LDGDISP"))=PAR("LDGDISP") S Y=""
 Q
