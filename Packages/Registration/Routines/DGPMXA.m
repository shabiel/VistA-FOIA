DGPMXA ; GENERATED FROM 'DGPM ASIH ADMIT' INPUT TEMPLATE(#452), FILE 405;3/28/13
 ;;5.3;Registration;**260005**;
 D DE G BEGIN
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:'$D(OLDA)
 S %=$P($G(OLD("DATE")),U) S:%]"" DE(2)=%
 S %=$P($G(OLD("WARD")),U) S:%]"" DE(4)=%
 S %=$P($G(OLD("ROOMBED")),U) S:%]"" DE(5)=%
 S %=$P($G(OLD("SHDIAG")),U) S:%]"" DE(10)=%
 S %=$P($G(OLD("ADMSCC")),U) S:%]"" DE(8)=%
 S %=$P($G(OLD("ADMREG")),U) S:%]"" DE(6)=%
 S %=$P($G(OLD("DATE")),U) S:%]"" DE(2)=%
 S %=$P($G(OLD("FDEXC")),U) S:%]"" DE(3)=%
 Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X2 S %DT="ST" D ^%DT K %DT S X=Y K:Y<1 X I $D(X),'$D(DGPMT) W !?3,*7,"USE BED CONTROL MOVEMENT OPTIONS!" K X
 Q
 ;
3 S DQ=3,DW="DIR;1",DV="SXR",DU="",DLB="DOES THE PATIENT WISH TO BE EXCLUDED FROM THE FACILITY DIRECTORY?",DIFLD=41
 S DE(DW)="C3^DGPMXA"
 S DU="0:NO;1:YES;"
 G RE
...
X3 I $D(X),'$D(DGPMT) D EN^DDIOL("USE BED CONTROL MOVEMENT OPTIONS!",,"!") K X
 ...
 Q
 ;
4 S DQ=4,DW="0;6",DV="R*P42'X",DU="",DLB="WARD LOCATION",DIFLD=.06
 S DE(DW)="C4^DGPMXA"
 S DU="DIC(42,"
 S X=+PAR("WARD")
 S Y=X
...
C4F1 Q
X4 Q
5 S DQ=5,DW="0;7",DV="*P405.4'X",DU="",DLB="ROOM-BED",DIFLD=.07
 S DE(DW)="C5^DGPMXA"
 S DU="DG(405.4,"
 S X=$S($G(PAR("ROOMBED")):+PAR("ROOMBED"),1:"")
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD:X="@",Z
...
C5F1 Q
X5 Q
6 S DQ=6,DW="0;12",DV="R*P43.4'",DU="",DLB="ADMITTING REGULATION",DIFLD=.12
 S DU="DIC(43.4,"
 G RE
X6 S DIC("S")="I '$P(^(0),""^"",4)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
7 S PAR("ADMREG")=$S(+Y>0:"",1:X_U)_Y,Y=U,DQ=7 D X7 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ K DG G OUT^DIE17
X7 I $S('$D(^DPT(DFN,.3)):1,$P(^(.3),"^",1)'="Y":1,1:0) S Y="@1"
 Q
8 S DW="0;11",DV="S",DU="",DLB="ADMITTED FOR SC CONDITION?",DIFLD=.11
 S DU="1:YES;0:NO;"
 G RE
X8 S PAR("ADMSCC")=$S(Y="^":"",X:1,1:0) Q
9 S DQ=10 ;@1
...
X10 K:$L(X)>30!($L(X)<3)!(X[";") X
 ...
 Q
 ;
11 S PAR("SHDIAG")=X,Y=U,DQ=11 G A
