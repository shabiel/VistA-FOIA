...
DR ;select input template for transaction type
 ...
 S:'DGPMN %=$$GETADM^DGPMAPI8(.OLD,MVTSD(DGPMY))
 ...
 N RE,API,RTN
 I DGPMN D  Q:RE=0
 . S API=$S(DGPMT=1:"ADMIT",DGPMT=2:"TRANF",DGPMT=3:"DISCH",DGPMT=6:"ADDFTS")
 . S RTN="S %=$$"_API_"^DGPMAPI"_DGPMT_"(.RE,.PAR)" X RTN
 . I RE=0 W !,$P(RE(0),U,2) Q
 I 'DGPMN D  Q:RE=0
 . S API=$S(DGPMT=1:"UPDADM",DGPMT=2:"UPDTRA",DGPMT=3:"UPDDSCH",DGPMT=6:"UPDFTS")
 . S RTN="S %=$$"_API_"^DGPMAPI"_DGPMT_"(.RE,.PAR,MVTSD(DGPMY))" X RTN
 . I RE=0 W !,$P(RE(0),U,2) Q
 Q
...
VAR ;Set up variables
 ;Modified in patch dg*5.3*692 to include privacy indicator node "DIR"
 S DA=DGPMDA,DGPMP=$S(DGPMN=1:"",1:$G(^DGPM(DA,0))_$S($G(^("DIR"))'="":U_^("DIR"),1:""),1:"") ;DGPMP=Before edit
