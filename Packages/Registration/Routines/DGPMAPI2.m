DGPMAPI2 ;RGI/VSL - TRANSFER PATIENT API; 3/4/2013
...
TRANSF1(RETURN,PARAM,TYPE,MAS,LMVT) ; Transfer patient
 ...
 D MVTEVT^DGPMAPI7(+PARAM("PATIENT"),2,TFN)
 S RETURN=TFN
 Q 1
 ;
ADDTRA(RETURN,PARAM) ; Add patient transfer
 ...
 S %=$$UPDPAT^DGPMAPI7(,.PARAM,+PARAM("PATIENT"))
 Q IFN2
 ;
TRANSF(RETURN,PARAM) ; Transfer patient
 ...
 D INITEVT^DGPMDAL1
 I 1 D
 . I TYPE=13 S %=$$ASIH^DGPMAPI6(.RETURN,.PARAM) Q
...
CHKUPD(RETURN,PARAM,TFN,OLD,NEW,MAS) ; Check transfer parameters
 ...
 . I 'RPHY,$D(PARAM("FCTY")),PARAM("ATNDPHY") S %=$$CHKRPM^DGPMAPI6(.RETURN,.PARAM) Q:'%
 S RETURN=($D(NEW)>0)
 Q 1
 ;
...
DELTRA(RETURN,TFN) ; Delete transfer
 ...
 D SETTEVT^DGPMDAL1(+TFN,RPHY,"A")
 S %=$$UPDPAT^DGPMAPI7(,.PARAM,+TRA(.03,"I"))
 D UPDPAT^DGPMAPI7(,.PARAM,+TRA(.03,"I"))
 D MVTEVT^DGPMAPI7(TRA(.03,"I"),2,+TFN)
