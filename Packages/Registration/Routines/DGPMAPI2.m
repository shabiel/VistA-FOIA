DGPMAPI2 ;RGI/VSL - TRANSFER PATIENT API; 4/8/13
...
EVT(DFN,TFN,MODE) ; Transfer event
 ...
 Q
CHKUDT(RETURN,PARAM,TFN,OLD,NEW,MAS) ; Check transfer date
 ; transfer date
...
CHKUPD(RETURN,PARAM,TFN,OLD,NEW,MAS) ; Check transfer parameters
 ...
 S %=$$CHKUDT(.RETURN,.PARAM,TFN,.OLD,.NEW,.MAS) Q:'RETURN 0
 ; type of movement
...
UPDTRA(RETURN,PARAM,TFN) ; Update transfer
 ...
 S %=$$LOCKMVT^DGPMAPI9(.RETURN,DFN) Q:'RETURN 0
 S %=$$UPD(.RETURN,.PARAM,TFN,.OLD,.NEW)
 S %=$$UPDPAT^DGPMAPI7(,.PARAM,DFN)
 D EVT(DFN,+TFN,1)
 D ULOCKMVT^DGPMAPI9(DFN)
 Q 1
 ;
UPD(RETURN,PARAM,TFN,OLD,NEW) ; Update transfer
