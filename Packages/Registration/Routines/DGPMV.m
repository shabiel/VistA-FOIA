DGPMV ;ALB/MRL/MIR - PATIENT MOVEMENT DRIVER; 1/10/2013
...
OREN S DGUSEOR=$$USINGOR()
 ...
 . S ERR(1)=" ",ERR(2)=$$EZBLD^DIALOG(4070000.165)
 . S ERR(3)=" ",ERR(4)=$$EZBLD^DIALOG(4070000.166)
 . D EN^DDIOL(.ERR)
 .R XTEMP:30
 .D DISPOQ K DGPMDER
MOVE ;
 ...
 G CHK:"^1^4^"[("^"_DGPMT_"^")
 I '$$HASMVT^DGPMAPI8(DFN,$S(DGPMT'=5:1,1:4)) D  Q
...
REG ;new patient
 ...
 ;
DIED S TXT(1)=$P(PAT("DTHDT"),U,2),%=$$ASK^DGPMVN(4070000.117,4070000.118,2,.TXT) Q:%=1  I % S DGPME=1 Q
 ;
