...
CANCEL ; Cancel Clinic postcards - Postcard #13
 ...
 W:$G(^TMP("R1ACSCRD",$J,"VIPP",1,"R1ACTEST","CNT")) !,"Number of patient postcards produced......"_+$G(^TMP($J,"VIPP","R1ACTEST","CNT"))
 K ^TMP("R1ACSCRD",$J)
 Q
 ;
 ;
CHKTST ;
 ...
 Q
 ;
MAIN2(SDDFN) ;Use ^SDAMA301 to retrieve Cancelled Clinics
 S R1ACPC=13
...
ACCHK ; Check postcard type "4" and see if it needs to be adjusted for #612418.4 matching
 ...
 .I $P($G(^DIZ(612418.4,X,0)),U,4)'="" S STRING=$P($G(^DIZ(612418.4,X,0)),U,4) I CNAME[STRING S PCARD=$P(^DIZ(612418.4,X,0),U,2) Q
