...
DQ U IO S POP=0
 ...
 ..S (R1ACPER,R1ACPERZ)="",R1ACPER=$P($G(^DIZ(612418,R1ACNUM,"AUDIT")),U) I $G(R1ACPER)>0 S R1ACPERZ=$E($P(^VA(200,R1ACPER,0),U),1,15)
 ..S (R1ACENT,R1ACENTZ)="",R1ACENT=$P($G(^DIZ(612418,R1ACNUM,"AUDIT")),U,2) I $G(R1ACENT)>0 S R1ACENTZ=$E(R1ACENT,4,5)_"/"_$E(R1ACENT,6,7)_"/"_$E(R1ACENT,2,3)
...
LOAD2 ;
 ...
 .D CHKL Q:R1ACFLGZ
 .W !,$E($P(R1ACNOD,U),1,23),?32,$P($G(R1ACNOD),U,2),?42,$P($G(R1ACNOD),U,3),?60,$P(R1ACNOD,U,4),?69,$P(R1ACNOD,U,5) S R1ACCNT=R1ACCNT+1
 .I $Y+15>IOSL,($E(IOST,1,2)="C-") S DIR(0)="E" D HDR S:POP R1ACFLGZ=1
 .I $Y+15>IOSL,($E(IOST,1,2)="C-") K DIR S DIR(0)="E" D ^DIR I $D(DTOUT)!(X[U)!($D(DUOUT)) S R1ACFLGZ=1 K DIR,DUOUT,DTOUT,DIRUT
...
KILL1 ;Deletes Boiler Plate in #612418.1 & all BLUEBOX TEMPLATES in #612418 pointing to that BP
 S R1ACDA=""
 F  S DIC="^DIZ(612418.1,",DIC(0)="AEMQ",DIC("A")="Select Boiler Plate to Delete:  " D ^DIC S R1ACDA=+Y D  I +Y<1 K DIC G EXIT2
 .I +R1ACDA<1 Q
 .S R1ACDAZ=0
...
KILL2 ;Deletes all blue box text for a given clinic in #612418
 N REF
 S REF=""
 F  S DIC="^DIZ(612418,",DIC(0)="AEMQ",DIC("A")="Slct Clinic where Blue Box Txt will be DELETED:  " D ^DIC S R1ACDA=+Y D  I +R1ACDA<1 K DIC G EXIT3
 .I +R1ACDA<1 Q
 .S DA(1)=R1ACDA,DA=0
