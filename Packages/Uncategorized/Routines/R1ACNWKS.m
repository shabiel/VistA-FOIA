...
DQ1 U IO S POP=0
 ...
 .S (R1ACRLL,R1ACELG,R1ACEMP)="",R1ACRLL=$P($G(^DPT(R1ACDFN,"ENR")),U) I $G(R1ACRLL)]"" S R1ACELG=$P($G(^DGEN(27.11,R1ACRLL,"E")),U) I $G(R1ACELG)]"" S R1ACEMP=$P(^DIC(8.1,R1ACELG,0),U) ;Patient's Eligibility Code
 .Q:R1ACELG="EMPLOYEE"  ;Exclude patients who are employees
 .Q:R1ACFLGZ  D CHKL1 Q:R1ACFLGZ
 .W !,$P($G(^DPT(R1ACDFN,0)),U),?31,$P($G(^DPT(R1ACDFN,0)),U,9),?42,R1ACDFN,?50,$E(R1ACEMP,1,8),?60,$E(R1ACADR2,1,19) S R1ACCNT=R1ACCNT+1
...
DQ2 U IO S POP=0
 ...
 W @IOF,! D HDR2
 S R1ACVA=$S($P($G(^DIC(4,R1ACIEN,1)),U)]"":$P($G(^DIC(4,R1ACIEN,1)),U),($P($G(^DIC(4,R1ACIEN,4)),U)]""):($P($G(^DIC(4,R1ACIEN,4)),U)),1:"")
 ...
 .I R1ACFLG=1 S R1ACBAI=$P($G(^DPT(R1ACDFN,.11)),U,16) S R1ACBAI=$S(R1ACBAI=1:"UNDELIVERABLE",R1ACBAI=2:"HOMELESS",R1ACBAI=3:"OTHER",1:"")
 .Q:R1ACFLGZ  D CHKL2 Q:R1ACFLGZ
 .I R1ACFLG=1 W !,$P($G(^DPT(R1ACDFN,0)),U),?31,$P($G(^DPT(R1ACDFN,0)),U,9),?42,R1ACDFN,?50,R1ACBAI,?70,$G(R1ACZFLG) S R1ACCNT=R1ACCNT+1,R1ACFLG=0,R1ACZFLG=""
 W !!,"Total Patient Count:  ",R1ACCNT
 G EXIT2
 Q
...
EXIT2 ;
 D ^%ZISC S:$D(ZTQUEUED) ZTREQ="@"
 K R1ACADR,R1ACFLG,R1ACFLGZ,R1ACPG,R1ACBAI,R1ACCTY,R1ACCNT,R1ACDFN,DUOUT,I,R1ACIEN,R1ACINS,R1ACNOW,POP,R1ACST,R1ACTMP,R1ACVA,X,Y,R1ACZIP,R1ACZFLG
...
DQ3 U IO S POP=0
 ...
 .I R1ACFLG=1 W !,$P($G(^DPT(R1ACDFN,0)),U),?31,$P($G(^DPT(R1ACDFN,0)),U,9),?42,R1ACDFN,?50,$G(R1ACADR) S R1ACCNT=R1ACCNT+1,R1ACFLG=0
 W !!,"Total Patient Count:  ",R1ACCNT
 G EXIT3
 Q
