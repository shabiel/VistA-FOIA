...
RPT ; produce report
 ...
 .S R1ACCIEN=""
 .N CCIEN
 .F  S R1ACCIEN=$O(^DIZ(612418,R1ACCIEN)) Q:R1ACCIEN=""  S CCIEN=$P($G(^(R1ACCIEN,0)),U) I CCIEN K ^TMP("R1ACSCIC",$J,1,CCIEN)
 .S R1ACCIEN=""
 .; build alphabetical array based upon clinic name
 .F  S R1ACCIEN=$O(^TMP("R1ACSCIC",$J,1,R1ACCIEN)) Q:R1ACCIEN=""  S ^TMP("R1ACSCIC",$J,1,"CLINICS",$O(^TMP("R1ACSCIC",$J,1,R1ACCIEN,"")))=R1ACCIEN
 .S R1ACCIEN=""
 .F  S R1ACCIEN=$O(^TMP("R1ACSCIC",$J,1,"CLINICS",R1ACCIEN)) Q:R1ACCIEN=""  D:R1ACLCNT>(IOSL-3) PHDR Q:$G(R1ACQUIT)  W !,R1ACCIEN_"  ("_^TMP("R1ACSCIC",$J,1,"CLINICS",R1ACCIEN)_")" S R1ACCCNT=$G(R1ACCCNT)+1,R1ACLCNT=$G(R1ACLCNT)+1
...
PHDR ; Page header
 ...
 I R1ACPCNT>1,($E($G(IOST),1,2)="C-") W !,"Press <ENTER> to continue or ""^"" to quit " R R1ACCONT:DTIME I R1ACCONT="^"!($T=0) S R1ACQUIT=1
 Q:$G(R1ACQUIT)
 W:R1ACPCNT>1 @IOF
 ...
 Q
 ;
CLNSTAT(X) ; test to see if valid clinic
 N R1ACSTAT
 S R1ACSTAT=1
