...
BBSTORE ; store new format
 ...
 Q
 ;
BBACT(R1ACACT) ; Edit or Reformat Blue Box
 Q:R1ACACT=""
...
BBINFO ;
 ...
 Q
 ;
CAL1(R1ACCLN) ; clinic address - called via a screenman form
 N Y,Z,R1ACCAD1,INST,DIVI
 S INST=+$P(^SC(+R1ACCLN,0),U,4),DIVI=+$P(^SC(+R1ACCLN,0),U,15),DIVI=$P($G(^DG(40.8,+DIVI,0)),U,2),R1ACCAD1=$S(INST>0:INST,DIVI>0:DIVI,1:"")
 S R1ACCAD1=$P($G(^DIC(4,R1ACCAD1,1)),U)
 Q R1ACCAD1
 ;
CAL2(R1ACCLN) ;  - called via a screenman form
 ...
 ;
DIVMAIL(R1ACDIV) ;
 I $$GET^DDSVAL("612418.7",R1ACDIV,1)="",$$GET^DDSVAL("612418.7",R1ACDIV,2)="",$$GET^DDSVAL("612418.7",R1ACDIV,3)="",$$GET^DDSVAL("612418.7",R1ACDIV,4)="",$$GET^DDSVAL("612418.7",R1ACDIV,5)="" D
