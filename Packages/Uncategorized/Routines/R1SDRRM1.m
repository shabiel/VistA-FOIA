...
EN K ^TMP($J) W ! S %DT="AEX",%DT("A")="Patient Recall date needing reprint: " D ^%DT Q:Y<0  S R1SDZSDT=Y K Y,X,%DT("A"),%DT
 ...
 I R1SDZSDT>R1SDDATE1!(R1SDZSDT=R1SDDATE1) W $C(7),!,"  The Date is Greater OR equal to what will be done during the Nightly Run - this Date must be less than" G EN
...
ENA S R1SDRRDTA1=0 F  S R1SDRRDTA1=$O(^TMP($J,"R1SDRR",R1SDCLINIC,"DILIST",R1SDRRDTA1)) Q:'R1SDRRDTA1  S R1SDRRDTA=$G(^TMP($J,"R1SDRR",R1SDCLINIC,"DILIST",R1SDRRDTA1,0)) D
...
EN1 D FIND^DIC(403.5,"","@;.01I;2.5;4;4.5I;5I;6I","QP",DFN,"","B","","","^TMP($J,""R1SDRR"",DFN)")
 S (R1SDC,R1SDI)=0 F R1SDI=0:0 S R1SDI=$O(^TMP($J,"R1SDRR",DFN,"DILIST",R1SDI)) Q:'R1SDI  I $D(^TMP($J,"R1SDRR",DFN,"DILIST",R1SDI,0)) S R1SDD=$G(^TMP($J,"R1SDRR",DFN,"DILIST",R1SDI,0)),R1SDC=R1SDC+1 S ^TMP("R1SDRR",$J,R1SDC)=R1SDI_"^"_R1SDD
 W !!!
 W !,"CHOOSE FROM:" F R1SDI=0:0 S R1SDI=$O(^TMP("R1SDRR",$J,R1SDI)) Q:'R1SDI   S R1SDCLINIC=$P($G(^TMP("R1SDRR",$J,R1SDI)),"^",6) D
 .W !,$J(R1SDI,4),">  "
 ...
 W !,"CHOOSE 1-",R1SDZ_" TO PRINT THE CARD" W:$D(^TMP("R1SDRR",$J,R1SDI+1)) !,"OR '^' TO QUIT" W ": " R X:DTIME I $S('$T!(X["^"):1,X="":1,1:0) S R1SDER=1 G QUIT
 S R1SDZ=X
